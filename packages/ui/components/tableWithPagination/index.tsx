    ...
    switch (data.type) {
      case 'status': {
        if (data.value === 'Approved') {
          ...
          return <Typography sx={tableWithPagination_style.green}>{data.value}</Typography>;
        } else if (data.value === 'Paid') {
          return <Typography sx={tableWithPagination_style.active}>{data.value}</Typography>;
      ...
      case 'icon': {
          ...
          <div style={{ display: 'flex' }}>
            ...
            {props?.view && (
              <IconButton onClick={() => props.handleIcon('view', main)}>
                <RemoveRedEyeOutlinedIcon sx={{ fontSize: '16px', color: '#98A0AC' }} />
              </IconButton>
            )}
            ...
            {props?.edit && (
              <Switch
                ...
                onChange={(e: any) => props.handleIcon('toggle', main, e.target.checked)}
              />
            )}
          </div>
        );
        ...
        let deleteIcon = (
          <div style={{ display: 'flex' }}>
            {props?.delete && (
              <IconButton onClick={() => props.handleIcon('delete', main)}>
                <DeleteOutlineIcon sx={{ fontSize: '16px', color: '#98A0AC' }} />
              </IconButton>
            )}
          </div>
        );
        ...
        let edittoggle = (
          <div style={{ display: 'flex' }}>
            {props?.edit && (
              <IconButton onClick={() => props.handleIcon('edit', main, props?.edit)}>
                <EditIMG color="#98a0ac" />
              </IconButton>
            )}
            {props?.edit && (
              <Switch checked={main.is_active} onChange={(e) => props.handleIcon('toggle', main, props?.edit)} />
            )}
          </div>
        );
        let viewedittoggle = (
          <div style={{ display: 'flex' }}>
            ...
            {props?.view && (
              <IconButton onClick={() => props.handleIcon('view', main, props?.view)}>
                <RemoveRedEyeOutlinedIcon sx={{ fontSize: '16px', color: '#98A0AC' }} />
              </IconButton>
            )}
            {props?.edit && (
        ...
        return (
          <div>
            {props?.edit && (
              <Checkbox
                sx={{ padding: '0px' }}
                onChange={(e: any) => props.handleIcon('checkBox', main, e?.target?.checked)}
                color="success"
                checked={data.value}
      ...
      case 'avatarText': {
        return (
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Avatar src={data.value.image} sx={{ width: 34, height: 34, lineHeight: 0, fontSize: 14 }}>
      ...
      case 'textLink': {
        return (
          <Box onClick={() => props?.onUnitClick(main, data)} sx={{ textDecorationLine: 'underline', color: 'blue' }}>
            {data.value}
          </Box>
        );
      }
      ...
      case 'PFImage': {
        return (
          <Box style={{ display: 'flex', alignItems: 'center' }}>
            <Avatar src={data.value !== '' ? data.value : '/images/pf.svg'} sx={{ width: 34, height: 34 }} />
          </Box>
        );
      }
      case 'logo': {
        return (
          <Box style={{ display: 'flex', alignItems: 'center' }}>
            <Avatar src={data?.value?.logo} sx={{ width: 34, height: 34, lineHeight: 0, fontSize: 14 }}>
              {data?.value?.name ? data?.value?.name?.charAt(0) : ''}
            </Avatar>
          </Box>
        );
      ...
      case 'link': {
        return (
          <Typography sx={tableWithPagination_style.link} onClick={() => props?.handleLink(data)}>
            {main.url}
          </Typography>
        );
      }
      case 'date': {
        return (
          <Typography sx={tableWithPagination_style.text}>
            {/* {data.value
      ...
      case 'more': {
        return (
          <Box style={{ display: props?.edit || props?.delete ? 'flex' : 'none' }}>
            <IconButton
            ...
            {index === data?.index && (
              <Menu anchorEl={anchorEl} open={open} onClose={handleClose} sx={tableWithPagination_style.menuList}>
                {props?.edit && (
                  <MenuItem
                    onClick={() => {
                      handleClose();
                ...
                {props?.edit && (
                  <MenuItem
                    ...
                    sx={tableWithPagination_style.menuItem}
                  >
                    {main.status === 'Active' || main.is_active === true ? 'In-active' : 'Active'}
                  </MenuItem>
                )}
      ...
      case 'unit': {
        return (
          <Box onClick={() => props?.onUnitClick(main, 'unit')} sx={{ textDecorationLine: 'underline', color: 'blue' }}>
            {data.value}
          </Box>
        );
      }
        ...
        return (
          <>
            {data.value === 'Close' ? (
              ...
              <Typography sx={tableWithPagination_style.inprogress}>In Progress</Typography>
            )}
          </>
        );
      }
      ...
      case 'more_7': {
        return (
          <div style={{ display: props?.view || props?.edit ? 'flex' : 'none' }}>
            <IconButton
              onClick={(e: any) => {
                e.stopPropagation();
                setAnchorEl(e.currentTarget);
          ...
          <div style={{ display: props?.edit ? 'flex' : 'none' }}>
            {props?.edit && (
              <Checkbox
                ...
                onChange={(e) => props.handleIcon('checkBox', data, e.target.checked)}
              />
            )}
          </div>
        );
      ...
      case 'more_appraisal': {
        return (
          <div style={{ display: props?.edit || props?.delete ? 'flex' : 'none' }}>
            <IconButton
              onClick={(e: any) => {
                e.stopPropagation();
                setAnchorEl(e.currentTarget);
      ...
      case 'action_edit': {
        return (
          <Box sx={tableWithPagination_style.editIcon} onClick={() => props?.handleIcon('edit', main)}>
            <EditIcon />
          </Box>
        );
      }
      ...
      {props?.rows?.length === 0 ? (
        ...
        <>
          ...
          >
            {/* <br /> */}
            {/* Table Component */}
            <Table sx={tableWithPagination_style.totalTable} stickyHeader aria-label="customized table">
              {/* heading */}
                                ...
                                <div
                                  ...
                                  onClick={() => {
                                    if (
                                      ...
                                      props?.view && props.handleIcon && props.handleIcon('view', row);
                                    }
                                  }}
                                >
