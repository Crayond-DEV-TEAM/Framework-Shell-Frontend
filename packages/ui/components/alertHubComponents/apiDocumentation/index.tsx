...
export function ApiDocumentation(props: ApiDocumentationProps): JSX.Element {
  const { requestBodyAPI, handleChangeCallback, apiBody, apiBodyMessage, apiBodyError, clearState } = useApiDocumentation()
  ...
  const handleChange = (name: string, value: string, apiBody: any) => {
                    ...
                    <Box key={index} sx={apiDocumentation_style.textBox}>
                      ...
                      {e === 'is_send_push_notification' || e === 'is_send_inapp_notification' || e === 'is_user_specific_notification' ? (
                        <>
                          <Checkbox
                            name={e}
                            onChange={(v) => handleChange(v?.target?.name, v?.target?.checked, apiBody)}
                            checked={apiBody[e]}
                        ...
                        <Input
                          name={e}
                          placeholder={e}
                          onChange={(e) => handleChange(e?.target?.name, e?.target?.value, apiBody)}
                          value={apiBody[e]}
