...
import { useAPIKey, useRouting, useSlug, useWebHookURL } from '../common';
      ...
      .then((response) => {
        ...
        if (Array.isArray(response.data.data.rows) && response.data.data.rows.length > 0) {
          response.data.data.rows.forEach((apiItem: SideMenuResponse) => {
            ...
            const WebHookUrl = useWebHookURL.getState().WebHookUrl;
            useWebHookURL.setState({
              WebHookUrl: {                
                ...WebHookUrl,
                [apiItem.service_name]: apiItem.webhook_url,
