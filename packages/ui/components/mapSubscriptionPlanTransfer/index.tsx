...
export const MapSubscriptionPlanTransfer = (props: MapSubscriptionPlanTransferProps): JSX.Element => {
  ...
  const onChange = (value: any, index: number, e: any) => {
    ...
    if (checked) {
      ...
      const updatedAddOnList = createEditSubscription.add_on.filter((item: any) => item.add_on.id !== value.add_on.id);
      ...
      const finding = Boolean(createEditSubscription.new_addon.find((item: any) => item.add_on.id === value.add_on.id));
      if (finding === false) {
